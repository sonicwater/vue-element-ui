...
<script>
import d3 from 'd3';
import D3_02_data from "./D3_02_data.js"

let dqData = D3_02_data.dqData;
...
"#B1EDA3","#71D2E1","#A954DC","#91DF6E","#CB6429","#D64ADC"];
    ...
    function draw(type){
        ...
        var svg =d3.select("#D3_2").select("."+type);
    ...
    // add title.
    d3.select("#D3_2").append("h3").text(data.title);
    
    // add svg and set attributes for distribution.
    d3.select("#D3_2").append("svg").attr("width",width+2*margin).attr("height",height+2*margin)
    ...
    // draw legends.
    var legRow = d3.select("#D3_2").append("div").attr("class","legend")
...
function drawAll(data, id){
    ...
    var seg = d3.select("#D3_2").selectAll("div").data(d3.range(data.length)).enter()
